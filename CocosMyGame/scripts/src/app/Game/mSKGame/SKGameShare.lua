local GameShare = class('GameShare')
...
function GameShare:share()
    ...
    my.scheduleOnce(function()
        audio.playSound("res/Game/GameSound/PublicSound/Snd_Camera.mp3")
    end)
...
function GameShare:screenShot()
    ...
    --cc.SpriteFrameCache:getInstance():addSpriteFrames("res/hallcocosstudio/images_New/plist/SaveBox_Img.plist")
    --local ico = cc.Sprite:createWithSpriteFrameName('Hall_IconGame.png')
    local ico = cc.Sprite:create('res/Game/GamePic/hall_share/lagt_launcher.png')
    ico:setScale(0.5)
    ico:setAnchorPoint(0,0)
    ico:setPosition(3,3)
...
function GameShare:show()
    ...
    if menu then
        ...
        --menu:setLocalZOrder(whi:getLocalZOrder() + 1)
        --menuClose:setPosition(1020,630)
        if size.height > 720 then
