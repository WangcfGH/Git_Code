...
function Base64:to_base64(to_encode)
    ...
    for i = 1, string.len(to_encode) do
        bit_pattern = bit_pattern .. self:to_binary(string.byte(string.sub(to_encode, i, i)))
    end
    ...
    for i = 1, string.len(bit_pattern), 6 do
        ...
        local offset = tonumber(self:from_binary(byte))
...
function Base64:from_base64(to_decode)
    ...
    for i = 1, string.len(unpadded) do
        ...
        bit_pattern = bit_pattern .. string.sub(self:to_binary(offset-1), 3)
    end

    for i = 1, string.len(bit_pattern), 8 do
        local byte = string.sub(bit_pattern, i, i+7)
        decoded = decoded .. string.char(self:from_binary(byte))
    end
