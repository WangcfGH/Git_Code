...
void initComponent(MainServer* mainSvr)
{
	...
	mainSvr->imGetIniFile = make_function_wrapper(spreDefine, &CPredefine::evGetIniFile);

    // chunkLogDB
	auto dbPool =  GetEntity().share_assign<GameDbPool>(8);
    std::weak_ptr<DBConnectPool> wDbPool = dbPool;
    mainSvr->evSvrStart += delegate(wDbPool, &DBConnectPool::OnServerStart);
    mainSvr->evShutdown += delegate(wDbPool, &DBConnectPool::OnServerStop);
	mainSvr->imDBOpera = make_function_wrapper(wDbPool, &DBConnectPool::dbInvokeByStrand<int, DBConnectEntry>);
    dbPool->imIniInt = make_function_wrapper(wpreDefine, &CPredefine::getInitDataInt);
    dbPool->imIniStr = make_function_wrapper(wpreDefine, &CPredefine::getInitDataString);
	...
	onlineClient->imGetGameID = make_function_wrapper(wpreDefine, &CPredefine::evGetGameID);
	onlineClient->imGetClientID = make_function_wrapper(wpreDefine, &CPredefine::evGetClientID);
    ...
    auto dataRecord = GetEntity().assign<DataRecord>();
	...
	dataRecord->imDBOpera = make_function_wrapper(dbPool, &DBConnectPool::dbInvokeByStrand<int, DBConnectEntry>);
    dataRecord->imGetIniInt = make_function_wrapper(wpreDefine, &CPredefine::getInitDataInt);
    onlineClient->evPlayerLogin += delegate(dataRecord, &DataRecord::OnPlayerLogon);
}
