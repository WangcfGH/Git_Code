...
void GameToChunklogClient::OnServerStart(BOOL& ret, TcyMsgCenter* msgCenter)
{
    if (ret)
    {
        std::string iniFile;
        ...
        if (tmpRet)
        {
            tmpRet = Initialize();
            if (tmpRet) {
                UwlTrace(_T("ChunkLogSockClient Initialize Success. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
                UwlLogFile(_T("ChunkLogSockClient Initialize Success. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
            } else{
                UwlTrace(_T("ChunkLogSockClient Initialize Failed. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
                UwlLogFile(_T("ChunkLogSockClient Initialize Failed. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
            }
        }
        else
        {
            UwlTrace(_T("ChunkLogSockClient Create Failed. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
            UwlLogFile(_T("ChunkLogSockClient Create Failed. ip[%s:%d]"), m_szIp.c_str(), m_nPort);
        }
    }
}

void GameToChunklogClient::OnShutdown()
{
    Shutdown();
}

BOOL GameToChunklogClient::ValidateClientInfo()
{
